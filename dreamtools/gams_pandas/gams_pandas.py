  ...
  def create_variable(self, name, index=None, explanatory_text="", data=None, dtype=None, copy=False, add_missing_domains=False):
    if index is not None:
      ...
      if is_iterable(data) and len(data) and is_iterable(data[0]):
        self.database.add_variable(name, len(data[0]), explanatory_text)
      elif is_iterable(data):
        self.database.add_variable(name, 1, explanatory_text)
      else:
        self.database.add_variable(name, 0, explanatory_text)
      self[name] = data
    return self[name]
